...
public class View {
    ...
    public USAStates chooseState() {
        displayHeader("USA States");
        int index = 1;
        for (USAStates state : USAStates.values()) {
            io.printf("%2d: %s%n", index++, state.getAbbr());
        }
        index--;
        String message = String.format("Select a State [1-%s]: ", index);
        return USAStates.values()[io.readInt(message, 1, index) - 1];
    }
    ...
    public void printDayKgReport(Map<Item, Double> map) {
        map.forEach((k, v) -> System.out.printf("%s: %.1f kg\n", k.getName(), v));
    }
    public void printDayValueByCategoryReport(Map<Category, BigDecimal> map) {
            map.entrySet().stream()
                    .filter(entry -> entry.getValue().compareTo(BigDecimal.ZERO) > 0)
                    .forEach(entry -> System.out.println(entry.getKey() + ": $ " + entry.getValue().setScale(2, RoundingMode.HALF_UP)));
    }
    ...
    public Forager makeForager() {
        ...
        forager.setState(chooseState().getAbbr());
        return forager;
    }
