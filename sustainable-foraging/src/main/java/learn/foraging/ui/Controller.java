...
public class Controller {
    ...
    private void viewForagers() {
        USAStates state = view.chooseState();
        List<Forager> foragers = foragerService.findByState(state.getAbbr());
        view.displayHeader(String.format("Foragers in %s", state.getName()));
        view.displayForagers(foragers);
        view.enterToContinue();
    }

    private void dayKgReport() {
        view.displayHeader(MainMenuOption.REPORT_KG_PER_ITEM.getMessage());
        LocalDate date = view.getForageDate();
        Map<Item, Double> report = forageService.getDayKgReport(date);
        view.printDayKgReport(report);
        view.enterToContinue();
    }
    private void dayValueByCategoryReport() {
        view.displayHeader(MainMenuOption.REPORT_CATEGORY_VALUE.getMessage());
        LocalDate date = view.getForageDate();
        Map<Category, BigDecimal> report = forageService.getDayValueByCategoryReport(date);
        view.printDayValueByCategoryReport(report);
        view.enterToContinue();
